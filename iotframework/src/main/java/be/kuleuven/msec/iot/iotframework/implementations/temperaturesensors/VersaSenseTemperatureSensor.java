...
public class VersaSenseTemperatureSensor extends TemperatureSensor {
    String id;
    String deviceMac;

    VersaSenseGateway gateway;
    VersaSenseTemperatureSensor thisSensor;

    public VersaSenseTemperatureSensor(String mac, String identifier, VersaSenseGateway gateway) {
                ...
                .subscribe(new Consumer<VersaSensePeripheralSampleJSONModel>() {
                    ...
                    public void accept(@NonNull VersaSensePeripheralSampleJSONModel versaSensePeripheralSampleJSONModel) throws Exception {
                        for (VersaSenseDataJSONModel data : versaSensePeripheralSampleJSONModel.getData()) {
                            if (data.getMeasurement().equals(VersaSense_constants.TEMPERATURE)) {
                                double value;
    ...
    public void monitorTemperature(final OnEventOccurred<Double> oeo) {
        gateway.getMonitoringService().subscribe(this.deviceMac, VersaSense_constants.TEMPERATURE, new OnEventOccurred<String>() {
            @Override
    ...
    public void unmonitorTemperature() {
        gateway.getMonitoringService().unsubscribe(this.deviceMac, VersaSense_constants.TEMPERATURE);
    }
